...
const (
	redisPrefix      = "game:%s:%v"
	redisClearPrefix = "game:%s:*"
)
...
func (r *RedisCache[K, T]) Get(id K) *T {
	key := fmt.Sprintf(redisPrefix, r.prefix, id)
...
func (r *RedisCache[K, T]) Clear() {
	keys, err := r.client.Keys(context.Background(), fmt.Sprintf(redisClearPrefix, r.prefix)).Result()
	if err != nil {
